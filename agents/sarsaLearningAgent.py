	...
	def learn(self):
		...
		while totalSteps < self.numIters:
			...
			while not done:
				...
				# SARSA-value update step
				nextAction = self.explorationPolicy.getAction(self.qDict[nextState], self.envObj)
				bootstrappedTarget = reward + self.discountFactor * self.qDict[nextState][nextAction]
